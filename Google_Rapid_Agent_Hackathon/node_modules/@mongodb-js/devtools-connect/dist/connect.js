...
exports.DevtoolsConnectionState = exports.MongoAutoencryptionUnavailable = void 0;
...
const aws_auth_compat_1 = require("./aws-auth-compat");
...
async function connectWithFailFast(uri, client, logger) {
    ...
    logger.emit('devtools-connect:connect-attempt-initialized', {
        uri,
        driver: (await client.options.metadata).driver,
...
function copyEventEmitterEvents(from, to) {
    from.emit = function (event, ...args) {
        to.emit(event, ...args);
...
class DevtoolsConnectionState {
    constructor(options, logger) {
        ...
        if (options.parentHandle) {
            ...
            const proxyingLogger = new events_1.EventEmitter();
...
async function connectMongoClient(uri, { useSystemCA = true, ...clientOptions }, logger, MongoClientClass) {
    ...
    try {
        ...
        if ((0, fast_failure_connect_1.isPotentialTLSCertificateError)(error)) {
...
async function connectMongoClientImpl({ uri, clientOptions, logger, MongoClientClass, useSystemCA, }) {
    ({ uri, clientOptions } = (0, aws_auth_compat_1.transformAWSAuthMechanismOptions)({
        uri,
        clientOptions,
    }));
    const cleanupOnClientClose = [];
    const runClose = async () => {
        let item;
        while ((item = cleanupOnClientClose.shift()))
            await item();
    };
    let ca;
    try {
        if (useSystemCA) {
            const { ca: caWithSystemCerts, asyncFallbackError, systemCertsError, systemCACount, messages, } = await (0, devtools_proxy_support_1.systemCA)({
                ca: clientOptions.ca,
        ...
        const addCAToProxyOptions = (opts) => {
            if (opts && (0, devtools_proxy_support_1.isExistingAgentInstance)(opts))
                return opts;
            return {
        ...
        const proxyAgent = clientOptions.proxy &&
            (0, devtools_proxy_support_1.useOrCreateAgent)(addCAToProxyOptions(clientOptions.proxy), clientOptions.applyProxyToOIDC === true ? undefined : 'mongodb://');
        ...
        const oidcFetch = (0, devtools_proxy_support_1.createFetch)(addCAToProxyOptions(oidcProxyOptions));
        clientOptions = {
            ...clientOptions,
            oidc: {
        ...
        if (proxyAgent && !hasProxyHostOption(uri, clientOptions)) {
            tunnel = (0, devtools_proxy_support_1.createSocks5Tunnel)(proxyAgent, 'generate-credentials', 'mongodb://');
        ...
        const mongoClientOptions = (0, lodash_merge_1.default)(clientOptions, shouldAddOidcCallbacks ? state.oidcPlugin.mongoClientOptions : {}, { allowPartialTrustChain: true }, ca ? { ca } : {});
        const customLookup = mongoClientOptions.lookup;
        mongoClientOptions.lookup = (hostname, options, callback) => {
            return (customLookup ?? dns_1.default.lookup)(hostname, { verbatim: false, ...options }, callback);
        };
